...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['new_profile_picture']) && isset($_POST['review_id']) && isset($_POST['current_profile_picture'])) {
    ...
    try {
        ...
        $query = "UPDATE reviews SET review_profilepicture = ? WHERE id = ?";
        $stmt = mysqli_prepare($link, $query);

        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "ss", $newFileName, $review_id);
